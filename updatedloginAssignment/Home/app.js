...
const userStatus = async () => {
  ...
  if (loggedInuser) {
    ...
    if (userData.status) {
      ...
      brandname.textContent=userData.data.username[0].toUpperCase()+userData.data.username.slice(1)
...
const postSubmitHandler = async () => {
  ...
  if (postimg.files[0]) {
    ...
    if (upload.status) {
      ...
      if(postimg.files[0].type.includes('video')){
        data.urlType='video'
...
const postDisplayHandler = async () => {
  ...
  if (postData.status) {
    ...
    let allPost = postData.data.map(async (e) => {
      let userDetail = (await getData(e.authorId, 'users')).data
    ...
    let postWithUserDetail = await Promise.all(allPost)
    postArea.innerHTML = ``
    allPostData=postWithUserDetail
    postWithUserDetail.forEach((e)=>{
         postArea.innerHTML+=`<div class="post-card">
    <div class="post-header" style="display: flex; justify-content: space-between;">
      <div>
      <img src="${(e.userData.profilePicture)?e.userData.profilePicture:'https://media.istockphoto.com/id/1495088043/vector/user-profile-icon-avatar-or-person-icon-profile-picture-portrait-symbol-default-portrait.jpg?s=612x612&w=0&k=20&c=dhV2p1JwmloBTOaGAtaA3AW1KSnjsdMt7-U_3EZElZ0=' }" alt="User Avatar" class="post-avatar">
      <strong>${e.userData.username}</strong> posted a status
    </div>
    ...
    <div class="post-content">
      ...
      ${(e.postDetail.urlType)?`<video controls  style="width: 100%;" height="300px"  alt="">
 
      <source src="${e.postDetail.imageUrl}" >
      
      </video>`:
       (e.postDetail.imageUrl)?`<img src="${e.postDetail.imageUrl}" style="width: 100%;" height="500px"  alt=""></img>`:``
...
// console.log(e)
  ...
  //   poseten.push(b)
  // })
  // const postWithData= Promise.all(poseten)
  // postRender(postWithData)
  // console.log(a)
  // console.log(a[0])
...
window.MypostRender = async() => {
  console.log(uid)
  let fileterdPost=await allPostData.filter((e)=>{
    return e.postDetail.authorId==uid
  })
  console.log(fileterdPost)

  postArea.innerHTML = ``
  fileterdPost.forEach((e)=>{
    postArea.innerHTML+=`<div class="post-card">
<div class="post-header" style="display: flex; justify-content: space-between;">
 <div>
 <img src="${(e.userData.profilePicture)?e.userData.profilePicture:'https://media.istockphoto.com/id/1495088043/vector/user-profile-icon-avatar-or-person-icon-profile-picture-portrait-symbol-default-portrait.jpg?s=612x612&w=0&k=20&c=dhV2p1JwmloBTOaGAtaA3AW1KSnjsdMt7-U_3EZElZ0=' }" alt="User Avatar" class="post-avatar">
...
<div class="post-content">
 ...
 ${(e.postDetail.urlType)?`<video controls  style="width: 100%;" height="200px"  alt="">
 
 <source src="${e.postDetail.imageUrl}" >
 
 </video>`:
  (e.postDetail.imageUrl)?`<img src="${e.postDetail.imageUrl}" style="width: 100%;" height="200px"  alt=""></img>`:``
